...
export default function ModalFormLaboratorio({ isOpen, onClose, formValues, callbackModalFormLaboratorio }) {
  const [inputsForm, setInputsForm] = useState({});
  const [radioButtons, setRadioButtons] = useState([]);
  const [aceptButtondisabled, setAceptButtondisabled] = useState(true);

  const { localStorageSession } = useAuth();
  const nuevoIngresoLaboratorio = async (values) => {
    ...
    await anexoService.registerControlsByEtapa(idEtapa, controlsAnexo).then((response) => {
      if (response.status == 200 || response.status == 201) {
        callbackModalFormLaboratorio(response.data.mensaje);
        onClose();
      } else {
        callbackModalFormLaboratorio("Ha ocurrido un problema al intentar ingresar el camión a Laboratorio.");
      }
    }).catch((error) => {
      callbackModalFormLaboratorio("Ha ocurrido un problema al intentar ingresar el camión a Laboratorio.");
    });
      ...
      <Modal isOpen={isOpen} onClose={onClose} scrollBehavior="inside">
        ...
        <Formik
          ...
          onSubmit={(values, { resetForm }) => {
            nuevoIngresoLaboratorio(values);
          }}
        >
          {({
            handleSubmit,
            ...
            <Form onSubmit={handleSubmit}>
              <ModalContent h="50rem" maxW="60rem" scrollBehavior="inside">
                <ModalHeader>Ingresando a Laboratorio</ModalHeader>
                <ModalCloseButton />
                <ModalBody>
                  <VStack
